...
import { AvatarDecoration, Badge, getBadges, getDecors, getUsers } from "../lib/api";
import { FETCH_COOLDOWN, SKU_ID } from "../lib/constants";
...
interface UsersDecorationsState {
    ...
    decorations: Map<string, AvatarDecoration>;
...
export const useUsersProfileStore = proxyLazy(() => zustandCreate((set: any, get: any) => ({
    ...
    decorations: new Map<string, AvatarDecoration>(),
    fetchDecorations: debounce(async () => {
        const { decorations } = get();
        const fetchedDecorations = await getDecors();
    ...
    try {
        ...
        if (AvatarDecoration) {
            ...
            if (!decoration) {
                ...
                const decoration = useUsersProfileStore.getState().decorations.get(AvatarDecoration);
                return { asset: AvatarDecoration, skuId: SKU_ID, animated: decoration.animated };
            }
            return { asset: AvatarDecoration, skuId: SKU_ID, animated: decoration.animated };
        }
        return null;
